...
class FirebaseData {
    ...
    func lisenLists(compl: @escaping(([String]) -> Void)) {
        ...
        db.collection("users").document(profile.id).addSnapshotListener {[weak self] (snaphot, _) in
            if let selF = self, selF.iUpdate == false, let data = snaphot?.data() {
                self?.profile = Profile(json: data)
                let list = self?.profile?.lists ?? profile.lists
                compl(list)
            }
        }
    }
